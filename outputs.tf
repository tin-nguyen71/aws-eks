...
output "cluster_iam_role_name" {
  ...
  value       = try(aws_iam_role.cluster_role[0].name, "")
}

output "cluster_iam_role_arn" {
  description = "IAM role ARN of the EKS cluster"
  value       = try(aws_iam_role.cluster_role[0].arn, "")
}

output "cluster_iam_role_unique_id" {
  description = "Stable and unique string identifying the IAM role"
  value       = try(aws_iam_role.cluster_role[0].unique_id, "")
}
...
output "secretmanager_eks_name" {
  description = "List of the autoscaling group names created by self-managed node groups"
  value       = try(aws_secretsmanager_secret.eks[0].name, "")
}
